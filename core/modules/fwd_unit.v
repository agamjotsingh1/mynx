...
module fwd_unit (
  ...
  output reg `W(`FWDLEN)     __id_fwdcsr,
  ...
  wire __id_fwdcsr_mem_wb = is_csrwrite_mem_wb && (__wb_csr == __id_csr);
  ...
  always @(*) begin
    ...
    if(__id_fwdcsr_mem_wb) __id_fwdcsr = `FWD_MEM_WB;
    else __id_fwdcsr = `FWD_NONE;
  end
endmodule
